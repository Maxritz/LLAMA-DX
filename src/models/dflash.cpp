...
void llama_model_dflash::load_arch_hparams(llama_model_loader & ml) {
    ...
    const std::string arch_prefix = ml.get_arch_name();
    if (!ml.get_arr(arch_prefix + ".target_layers", target_layer_ids, false) &&
        !ml.get_arr(arch_prefix + ".dflash.target_layer_ids", target_layer_ids, false)) {
...
void llama_model_dflash::load_arch_tensors(llama_model_loader & ml) {
    ...
    const auto dim0_or = [&](llm_tensor t, const char * suffix, int64_t fallback) -> int64_t {
        const ggml_tensor * meta = ml.get_tensor_meta(tn(t, suffix).str().c_str());
        return meta ? meta->ne[0] : fallback;
    };
    const auto dim1_or = [&](llm_tensor t, const char * suffix, int64_t fallback) -> int64_t {
        const ggml_tensor * meta = ml.get_tensor_meta(tn(t, suffix).str().c_str());
        return meta ? meta->ne[1] : fallback;
    };
    const auto dim0_or_l = [&](llm_tensor t, const char * suffix, int layer, int64_t fallback) -> int64_t {
        const ggml_tensor * meta = ml.get_tensor_meta(tn(t, suffix, layer).str().c_str());
        return meta ? meta->ne[0] : fallback;
    };
    const auto dim1_or_l = [&](llm_tensor t, const char * suffix, int layer, int64_t fallback) -> int64_t {
        const ggml_tensor * meta = ml.get_tensor_meta(tn(t, suffix, layer).str().c_str());
        return meta ? meta->ne[1] : fallback;
    };
    ...
    fc = create_tensor(tn(LLM_TENSOR_FC, "weight"), { n_embd_inp, n_embd }, TENSOR_NOT_REQUIRED);
    if (!fc) {
        fc = create_tensor(tn(LLM_TENSOR_DFLASH_FC, "weight"), { n_embd_inp, n_embd }, 0);
    }
    output_norm_enc = create_tensor(tn(LLM_TENSOR_ENC_OUTPUT_NORM, "weight"), { n_embd }, TENSOR_NOT_REQUIRED);
    if (!output_norm_enc) {
        output_norm_enc = create_tensor(tn(LLM_TENSOR_DFLASH_HIDDEN_NORM, "weight"), { n_embd }, 0);
    }

    if (decoder_laguna) {
        aux_norm = create_tensor(tn(LLM_TENSOR_ENC_AUX_NORM, "weight"), { n_embd, (int64_t) target_layer_ids.size() }, 0);
    } else {
        create_tensor(tn(LLM_TENSOR_DFLASH_AUX_NORM, "weight"), {dim0_or(LLM_TENSOR_DFLASH_AUX_NORM, "weight", n_embd), dim1_or(LLM_TENSOR_DFLASH_AUX_NORM, "weight", 1)}, TENSOR_NOT_REQUIRED);
    }
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        if (decoder_laguna) {
            ...
            layer.wqkv_gate = create_tensor(tn(LLM_TENSOR_ATTN_GATE, "weight", i), {dim0_or_l(LLM_TENSOR_ATTN_GATE, "weight", i, n_embd), dim1_or_l(LLM_TENSOR_ATTN_GATE, "weight", i, n_head)}, TENSOR_NOT_REQUIRED);
        }

        layer.ffn_norm = create_tensor(tn(LLM_TENSOR_FFN_NORM, "weight", i), { n_embd }, TENSOR_NOT_REQUIRED);
        if (!layer.ffn_norm) {
            layer.ffn_norm = create_tensor(tn(LLM_TENSOR_ATTN_POST_NORM, "weight", i), { n_embd }, 0);
        }
...
llama_model_dflash::graph<false>::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    if (ubatch.embd) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            if (inp_attn && inp_attn->self_k_rot) {
                Kcur = ggml_mul_mat_aux(ctx0, Kcur, inp_attn->self_k_rot);
            }
            if (inp_attn && inp_attn->self_v_rot) {
                Vcur = ggml_mul_mat_aux(ctx0, Vcur, inp_attn->self_v_rot);
            }
            if (inp_attn_iswa && hparams.is_swa(il)) {
                if (inp_attn_iswa->self_k_rot_swa) {
                    Kcur = ggml_mul_mat_aux(ctx0, Kcur, inp_attn_iswa->self_k_rot_swa);
                }
                if (inp_attn_iswa->self_v_rot_swa) {
                    Vcur = ggml_mul_mat_aux(ctx0, Vcur, inp_attn_iswa->self_v_rot_swa);
                }
            }
            if (inp_attn_iswa && !hparams.is_swa(il)) {
                if (inp_attn_iswa->self_k_rot) {
                    Kcur = ggml_mul_mat_aux(ctx0, Kcur, inp_attn_iswa->self_k_rot);
                }
                if (inp_attn_iswa->self_v_rot) {
                    Vcur = ggml_mul_mat_aux(ctx0, Vcur, inp_attn_iswa->self_v_rot);
                }
            }

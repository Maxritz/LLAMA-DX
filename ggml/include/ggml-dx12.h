...
struct ggml_backend_buffer_type * ggml_backend_dx12_buffer_type(struct ggml_backend * backend);

// Get a host (pinned) buffer type for fast CPU<->GPU transfers
struct ggml_backend_buffer_type * ggml_backend_dx12_host_buffer_type(void);

// Get the ggml_backend_dev_t for a specific adapter
struct ggml_backend_device * ggml_backend_dx12_get_device(int32_t adapter_index);
...
int ggml_backend_dx12_get_caps(struct ggml_backend * backend, ggml_backend_dx12_caps * caps);

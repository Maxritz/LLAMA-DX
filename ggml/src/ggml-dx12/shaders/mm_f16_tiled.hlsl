/*
 * mm_f16_tiled.hlsl
 * PURPOSE: Tiled 32x32 GEMM with shared-memory tiling, F16 weights x F32 activations -> F32
 *
 * 16x16 threads per group, each thread computes a 2x2 sub-block of the 32x32 output tile.
 * Shared memory tiles (As[32][32] + Bs[32][32], 8KB total) cut global memory reads by ~32x.
 */
...
void main(uint3 gtid : SV_GroupThreadID, uint3 gid : SV_GroupID) {
    ...
    for (uint kt = 0; kt < params.K; kt += 32) {
        ...
        // Zero this thread's shared memory entries (otherwise stale data from
        // previous K-tile persists when bounds check skips the load below)
        ...
        // Inner tile matmul: acc[r][c] = sum_k As[gtid.x*2 + c][k] * Bs[gtid.y*2 + r][k]
